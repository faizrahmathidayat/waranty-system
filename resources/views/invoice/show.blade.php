...
@section('content')
<script>window.invoicePaymentConfig={grandTotal:{{ (float) $invoice->grand_total }},paid:{{ (float) $invoice->paid_amount }},outstanding:{{ (float) $invoice->outstanding_amount }},canVoidPayment:{{ optional($invoice->order)->warranties->isEmpty() ? 'true' : 'false' }},payments:@json($invoice->payments->map(fn($payment)=>['id'=>$payment->id_payment,'status'=>$payment->status ?? 'ACTIVE'])->values()) };</script>
...
<section class="content" style="padding-top:17px"><div class="container-fluid"><div class="card"><div class="card-header"><h3 class="card-title">INVOICE — {{ $invoice->invoice_number }}</h3><div class="card-tools"><a href="{{ url('/invoice') }}" class="btn btn-sm btn-secondary">Kembali</a> <a target="_blank" href="{{ url('/invoice/print/'.$invoice->id_invoice) }}" class="btn btn-sm btn-primary">Print</a>@if($invoice->status !== 'CANCELLED') <button class="btn btn-sm btn-danger invoice-cancel" data-id="{{ $invoice->id_invoice }}">Cancel</button>@endif</div></div><div class="card-body"><div class="row"><div class="col-md-4"><strong>Customer</strong><br>{{ optional($invoice->customer)->nama_customer }}<br>{{ optional($invoice->customer)->alamat }}<br>{{ optional($invoice->customer)->no_hp }} | {{ optional($invoice->customer)->email }}</div><div class="col-md-4"><strong>Invoice</strong><br>Date: {{ $invoice->invoice_date->format('d M Y') }}<br>Due: {{ optional($invoice->due_date)->format('d M Y') ?? '-' }}<br>Status: <span class="badge badge-info">{{ $invoice->status }}</span></div><div class="col-md-4"><strong>Order</strong><br>{{ optional($invoice->order)->order_number }} — {{ optional($invoice->order)->order_type }}<br>@if(optional($invoice->order)->order_type==='BUILDING'){{ optional(optional($invoice->order)->building)->nama_bangunan }} — {{ optional(optional($invoice->order)->building)->alamat }}@else{{ optional(optional($invoice->order)->vehicle)->no_polisi }} {{ optional(optional($invoice->order)->vehicle)->merk }} {{ optional(optional($invoice->order)->vehicle)->model }}@endif</div></div><hr><div class="table-responsive"><table class="table table-bordered"><thead class="bg-secondary"><tr><th>No</th><th>Description</th><th>Area / Luas</th><th>Qty</th><th>Unit Price</th><th>Discount</th><th>Subtotal</th></tr></thead><tbody>@foreach($invoice->items as $item)<tr><td>{{ $loop->iteration }}</td><td>{{ $item->treatment_name }} - {{ $item->product_name }}{{ $item->variant_name ? ' - '.$item->variant_name : '' }}</td><td>{{ $item->area }}@if(optional($invoice->order)->order_type === 'BUILDING')<br><small>Luas: {{ $item->total_luas }} m²</small>@elseif($item->panjang)<br><small>{{ $item->panjang }} × {{ $item->lebar }} m; {{ $item->total_luas }} m²</small>@endif</td><td>{{ $item->quantity }} {{ $item->unit }}</td><td>Rp {{ number_format($item->unit_price,2,',','.') }}</td><td>Rp {{ number_format($item->discount,2,',','.') }}</td><td>Rp {{ number_format($item->subtotal,2,',','.') }}</td></tr>@endforeach</tbody></table></div><div class="row justify-content-end"><div class="col-md-4"><table class="table"><tr><th>Subtotal</th><td class="text-right">Rp {{ number_format($invoice->subtotal,2,',','.') }}</td></tr><tr><th>Discount</th><td class="text-right">Rp {{ number_format($invoice->discount,2,',','.') }}</td></tr><tr><th>Tax</th><td class="text-right">Rp {{ number_format($invoice->tax_amount,2,',','.') }}</td></tr><tr><th>Grand Total</th><td class="text-right font-weight-bold">Rp {{ number_format($invoice->grand_total,2,',','.') }}</td></tr><tr><th>Paid</th><td class="text-right">Rp {{ number_format($invoice->paid_amount,2,',','.') }}</td></tr><tr><th>Outstanding</th><td class="text-right">Rp {{ number_format($invoice->outstanding_amount,2,',','.') }}</td></tr></table></div></div></div></div></div></section>
